...
def window_sensitivity(fs=500, duration=10.0):
    ...
    for N in window_sizes:
        ...
        delta_f.append(fs/N)           
        hr_estimates.append(heart_rate_dft(segment, fs))       #estimates BPM 
...
def noise_robustness(fs=500, duration=10.0, n_trials=8):
    ...
    true_bpms = {"normal": 70.0, "bradycardia": 30.0, "arrhythmia": 140.0}  #BPM
    ...
    for name, gen_fn in generators.items():
        ...
        hr_at_snr = []      #BPM, estimated heart rate at each SNR
...
def test_failure_cases(fs=500):     #Hz
    ...
    win_len = 256       #samples  (~0.512 s at fs=500 Hz)
    hop = 64        #step size between consecutive frames)
